...
pgtotal=$(echo $pginfo | sed -n '/pgs/s/.* \([0-9]*\) pgs.*/\1/p')
...
function ceph_osd_down()
{
  OSD_DOWN=$($ceph_bin osd dump |grep "^osd"| awk '{print $1 " " $2 " " $3}'|grep down|wc -l)
 # COUNT=$(echo "scale=2; $OSD_DOWN*100/$ceph_osd_count" |bc)
  if [[ "$OSD_DOWN" != "" ]]
  then
    echo $OSD_DOWN
...
function ceph_mon_get_down()
{
  DOWN=$($ceph_bin status|sed -n 's/\([0-9]*\) mons down.*/\1/p')
  if [[ "$DOWN" != "" ]]
  then
    echo $DOWN
  else
...
function pool_total()
{
  POOL=$($ceph_bin status|sed -n '/pools/s/.* \([0-9]*\) pools.*/\1/p')
  if [[ "$POOL" != "" ]]
  then
    echo $POOL
  else
